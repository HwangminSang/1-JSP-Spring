...
<script>
	...
	$(document).ready(
			...
			function() {
				...
				findCertificateRequestList('모든부서'); // 초기실행일 경우 ,  오늘날짜로 검색
	...
	function showCertificateListGrid() {
		...
		gridOptions = {
			...
			var cash = node.data.cash; // ? 이건무
			var profile = node.data.receipt;
			console.log(cash+profile);
		document.getElementById('profileImg').setAttribute("src","${pageContext.request.contextPath}/profile/"+cash+"."+profile);//사진갖고오는기능
			//document.getElementById('profileImg').setAttribute("src","${pageContext.request.contextPath}/profile/16,000.jpg");//사진갖고오는기능
		},
	...
	function findCertificateRequestList(allDept) {
		...
		if (allDept == "모든부서") {  //넘어온값이 '모든부서'와 같다면 
			...
			startDate = rrrr + "-" + mm + "-" + dd; //시작일을 오늘날짜로 넘긴다
				...
				$.ajax({
					url : "${pageContext.request.contextPath}/certificate/proofApproval.do",
					data : {
						"method" : "findProofAttdListByDept",
						"deptName" : deptName,
						"startDate" : startDate,
						"endDate" : endDate,
					},
	...
	function modifyCertificateRequest() {
		var sendData = JSON.stringify(proofAttdList); // 그리드의 모든애들을 보낸다

		$.ajax({
			url : "${pageContext.request.contextPath}/certificate/proofApproval.do",
			data : {
				"method" : "modifyProofList",
				"sendData" : sendData
			},
			dataType : "json",
			success : function(data) {
	...
	function applovalCertificateRequest() {
		var rowNode = gridOptions.api.getSelectedNodes();  // [ {} ] 리턴
		...
		rowNode[0].setDataValue('applovalStatus', "승인");   // ==rowNode.setData 키 , 값 //https://www.ag-grid.com/javascript-data-grid/data-update-single-row-cell/#view-refresh
		//rowNode[0].setDataValue('rejectCause', "");
		rowNode[0].setDataValue('status', "update"); //자동새로고침이 된다. 렌더링된다.
		console.log(rowNode[0].data);
	}
	/* 승인취소버튼 함수 */
	...
	function getCode(code, inputText, inputCode) {
		...
		window.open("${pageContext.request.contextPath}/base/codeWindow.html?code=" + code + 
				"&inputText=" + inputText + "&inputCode="+ inputCode, "newwins", option);
	}
</script>
</head>
<body>
		<section id="tabs" style="width:770px; height:700px;" class="wow fadeInDown">
			...
			증빙종류<input type=text id="search_certificate_deptName" readonly>
			...
			<br />
			<br />
			<div id="certificateList_grid" style="height: 400px; width: 700px"
				class="ag-theme-balham"></div>
			<div id="certificateList_pager"></div>
	</section>
	<td>
		<section id="tabs" style="width:600px; height:400px;" class="wow fadeInDown">
		<div class="container">
			...
			<hr style="background-color:white; height: 1px;">
				...
				<div style=" display: inline-block; position:absolute; margin-left: 50px; margin-top: 30px;" >  <!-- VIEWPORT가 기준 -->

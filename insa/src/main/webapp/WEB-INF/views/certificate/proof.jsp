...
<script>
	...
	$(document).ready(function() {
				...
				$("#selectproofAttd").click(
						function() {
							getCodeRest("ROF001", "ROF002", "ROF003",  //교육 , 기타 ,  출장
				...
				$("#delete_proofAttd_Btn").click(function() { // 
					var flag = confirm("선택한 증빙외신청을 정말 삭제하시겠습니까?");
					if (flag)
						removeRestAttdList();
				});
				...
				/* 사용자 기본 정보 넣기 */
				$("#proofAttd_emp").val("${sessionScope.name}");
				$("#proofAttd_dept").val("${sessionScope.dept}");
				$("#proofAttd_position").val("${sessionScope.position}");
				$("#proofAttd_cash").bind('keyup keydown', function() { // 먼가를 입력하면 실행됨 .  금액 
					inputNumberFormat(this);
				});
			
	
				$("#proof_img_form").ajaxForm({  //form 태그로 ajaxform 이용 파일 업로드
					dataType : "json",
	...
	//콤마찍기 //
	function comma(str) {  //정규식을 사용하기위해 문자열로 바꿔준다.  .toString도 가능
		str = String(str);
		return str.replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,');  //우리는 1000단위로 끊어, 콤마(,)를 삽입해야 한다
	}//정규식 리터럴(슬래쉬/로 감싸는 패턴)을 사용하는 방

	//콤마풀기
	function uncomma(str) {  //https://blog.outsider.ne.kr/141
		str = String(str); //문자열로 바꿈
		return str.replace(/[^\d]+/g, ''); // 숫자가 아닌 것들을 전체에서 찾아서 ''로 바꾼다
	}
	...
	function findrestAttdList() {
		...
		$.ajax({
					url : "${pageContext.request.contextPath}/certificate/receiptProof.do",
					data : {
						"method" : "proofLookupList",
						"empCode" : empCode,
						"startDate" : startVar,
						"endDate" : endVar,
						"code" : code
					...
					success : function(data) {
						...
						showRestAttdListGrid(); // 받아온걸 뿌리기위해 그리드를 다시 불러서 받아온데이터를 다시 준다.
					}
				});
	}
	...
	function comma(num) { // 13000  금액부분 , 생기게하기
		var len, point, str;

		num = num + ""; //문자열로바꿈
		point = num.length % 3;  //5자리일경우 2  즉 나머지를 준다
		len = num.length;// 5

		str = num.substring(0, point); // STR = 13
		while (point < len) { //2<5
			if (str != "")
				str += ","; //13,
			str += num.substring(point, point + 3);  // 2,5   13,000
			point += 3; //2+3 == 5 while문 나옴
		}
	...
	function readURL(input) {  //https://developer.mozilla.org/ko/docs/Web/HTML/Element/Input/file

		if (input.files && input.files[0]) {   // FILE 속성에서 FILES 라는 배열안에 내가선택한 파일을 넣음.  그래서 파일이 있나없나 확인하는과정
			var reader = new FileReader(); //파일을 읽어옴
			reader.onload = function(e) {
				// 이미지 Tag의 SRC속성에 읽어들인 File내용을 지정 (아래 코드에서 읽어들인 dataURL형식)
				$("#profileImg").attr("src", e.target.result);  //파일을 읽어올수있도록 src속성에 넣느다.
	...
	function removeRestAttdList() {
		var selectedRowData = gridOptions.api.getSelectedRows(); //  [{}] 형식으로 가져온다 . 즉 체크한놈들을 가져옴 
	...
	function getCodeRest(code1, code2, code3, inputText, inputCode) {
		...
		window.open("${pageContext.request.contextPath}/base/codeWindow.html?code1="
	...
	function registrestAttd() {
		...
		var proofList = {
			"empCode" : empCode, //직원코드
			"proofTypeCode" : $("#selectProofAttdCode").val(), // 출장 교육 등의 코드번호
			"proofTypeName" : $("#selectproofAttd").val(), //출장
			"startDate" : $("#proofAttd_startD").val(), //시작날짜
			"position" : $("#proofAttd_position").val(), // 직위
			"dept" : $("#proofAttd_dept").val(), //부서
			"cash" : $("#proofAttd_cash").val(), //금액 14,000
			"cause" : $("#proofAttd_cause").val(), //사유
			"receipt" : $("#proofAttd_receipt").val(), // 영수증사진
			"applovalStatus" : '승인대기'
		};
		...
		$.ajax({
					...
					success : function(data) {
						if (data.errorCode < 0) {
							...
							console.log("여기???")
							if($("#proofAttd_receipt").val()!=""){ // 즉 파일이 있다면
			  					console.log($("#proofAttd_receipt").val());
			  					console.log("ㅁㅁㅁㅇㅇㄴㅇㅁㄴㅇ?")
			  					
			  					$("#proof_img_cashCode").val(proofList.cash);  // img코드로 캐쉬 금액을 넣네??
							    $("#proof_img_form").submit();// 사진을 form태그로 넘겨서 입력시키기
							}  
							
							alert("신청되었습니다");  // 먼저 데이터를 (insert)넣고 그리고 다시 사진을 넣어서 사진쪽이 UPDATA다
						}
						location.reload();
					}
				});
...
<body> 
	<section id="tabs" class="wow fadeInDown" style="height:700px; width:850px;">
	...
	<div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
		<div class="tab-pane fade show active" id="employmentRequest_tab" role="tabpanel" aria-labelledby="nav-home-tab">
			<font>증빙외 구분 </font> 
			<input id="selectproofAttd"class="ui-button ui-widget ui-corner-all" readonly> 
			<input type="hidden" id="selectProofAttdCode" name="restAttdCode">
			<hr>
			<table>
				<tr>
					<td><font>신청자 </font></td>
				...
				<tr>
					...
					<form id="proof_img_form" action="${pageContext.request.contextPath}/base/proofImg.do" enctype="multipart/form-data" method="post">
						<input type="hidden" name="cashCode" id="proof_img_cashCode">
						<input type="file" name="proofImgFile" id="proofAttd_receipt" style="display: none;" onChange="readURL(this)"/>
